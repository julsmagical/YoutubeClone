...
| `JWT\_ISSUER` | `Jwt:Issuer` | Emisor del token JWT |

| `JWT\_AUDIENCE` | `Jwt:Audience` | Audiencia del token JWT |

| `JWT\_PRIVATE\_KEY` | `Jwt:PrivateKey` | Clave secreta para firmar tokens |

| `JWT\_EXPIRATION\_IN\_MINUTES\_MIN` | `Jwt:ExpirationInMinutesMin` | Expiración mínima del token (minutos) |

| `JWT\_EXPIRATION\_IN\_MINUTES\_MAX` | `Jwt:ExpirationInMinutesMax` | Expiración máxima del token (minutos) |
